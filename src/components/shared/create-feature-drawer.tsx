...
export const CreateFeatureDialog = ({
    ...
    const mutation = useMutation({
        ...
        onSuccess: () => {
            ...
            toast.success("Feature proposed successfully");
            onClose();
            setName(""); setDescription(""); setPriority("medium"); setDueDate(""); setAssignee("");
        },
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!name || !description || !dueDate) {
            toast.error("Please fill in Title, Description and Target Date");
            return;
        }
        mutation.mutate({ name, description, priority, dueDate: new Date(dueDate), assignee: assignee || undefined, projectId, blobId });
    };
                ...
                <DrawerHeader className="border-b border-border px-6 py-5 shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="space-y-0.5 text-left">
                            <DrawerTitle className="text-lg font-black tracking-tight text-violet-500">Propose Feature</DrawerTitle>
                            <p className="text-[10px] text-muted-foreground font-semibold uppercase tracking-widest">
                                New capability in <span className="text-violet-500 font-black">{blobName}</span>
                            </p>
                        </div>
                    </div>
                </DrawerHeader>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
                    <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
                        <div className="space-y-1.5">
                            <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Feature Title</Label>
                            <Input
                                placeholder="What should be implemented?"
                                value={name}
                                onChange={(e) => setName(e.target.value)}
                                className="rounded-xl h-11 font-medium bg-secondary/50 border-border focus-visible:ring-violet-500/30"
                            />
                        </div>
                        ...
                        <div className="space-y-1.5">
                            ...
                            <Textarea
                                ...
                                className="rounded-xl min-h-[140px] font-medium resize-none bg-secondary/50 border-border focus-visible:ring-violet-500/30"
                            />
                        </div>
                        ...
                        <div className="space-y-1.5">
                            <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Responsible Agent <span className="opacity-40">(Optional)</span></Label>
                                        ...
                                        <SelectItem key={member.userId || member.id || member._id} value={member.userId || member.id || member._id}>
                                            <div className="flex items-center gap-2 py-0.5">
                                                <Avatar className="h-6 w-6">
                                                    ...
                                                    <AvatarFallback className="text-[10px] font-black bg-violet-500/10">{member.name?.slice(0, 2).toUpperCase()}</AvatarFallback>
                                                </Avatar>
                    ...
                    <DrawerFooter className="border-t border-border px-6 py-4 flex-row justify-between shrink-0">
                        ...
                        <Button
                            ...
                            className="rounded-xl px-8 h-11 font-black bg-violet-600 hover:bg-violet-700 text-white shadow-lg shadow-violet-500/20"
                        >
                            {mutation.isPending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Saving...</> : "Propose Feature"}
                        </Button>
                    </DrawerFooter>
                </form>
            </DrawerContent>
